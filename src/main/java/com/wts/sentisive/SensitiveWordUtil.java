...
public class SensitiveWordUtil {
    ...
    public static void main(String[] args) {
        ...
        Set<String> sensitiveWordSet = new HashSet<String>(2800);
        for (int i = 0; i < 2000; i++) {
            sensitiveWordSet.add(i + "");
        }
        ...
        sensitiveWordSet.add("中国人民");
        sensitiveWordSet.add("比");
        sensitiveWordSet.add("中华民族");
        //初始化敏感词库
...
//        System.out.println("敏感词的数量：" + SensitiveWordUtil.sensitiveWordMap.size());
        String string = "比太多中国人的伤感情怀也许只局限于饲养基地 荧幕中的情节。"
                + "然后我们的扮演的角色就是跟随着主人公的喜红客联盟 怒哀乐而过于牵强的把自己的情感也附加于银幕情节中，然后感动就流泪，"
                + "难过就躺在某一个人的怀里尽情的阐述心扉或者手机卡复制器一个贱人一杯红酒一部电影在夜 深人静的晚上，关上电话静静的发呆着。";
//        String string = "逼我爱中国人民共和国中华民族人民";

        System.out.println("待检测语句字数耗时：" + string.length());

        //是否含有关键字
        start = System.currentTimeMillis();
        boolean result = SensitiveWordUtil.contains(string);
        System.out.println("检测是否存在敏感词耗时: " + (System.currentTimeMillis() - start));
